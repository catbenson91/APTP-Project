...
opd_data_grouping %>% 
  ...
  summarize(Mean_Response = hms::as.hms(mean(response_time, na.rm = TRUE,)),
            count = n()) %>% 
  filter(count >= 5) %>% 
  arrange(Mean_Response) -> mean_response_grouping
...
opd_data_grouping %>% 
  ...
  summarize(Mean_Response = hms::as.hms(mean(response_time, na.rm = TRUE)),
            count = n()) %>% 
  arrange(desc(Mean_Response)) -> mean_response_beat


opd_data_grouping %>% 
  filter(Source == "911 CALL", Priority %in% c(0, 1, 2, 3, 4)) %>% 
  group_by(Beat) %>% 
  summarize(Mean_Response = mean(minute(response_time), na.rm = TRUE),
            count = n()) %>% 
  arrange(desc(Mean_Response)) 

opd_data_grouping %>% 
  min = hour(response_time)*60 + minute(response_time)

opd_data_grouping$response_time <- as.duration(opd_data_grouping$response_time) 
...
opd_data_grouping$response_time <- hms::as.hms(opd_data_grouping$response_time)


opd_data_grouping %>% 
  filter(Source == "911 CALL", Grouping == "Assault") %>% 
  group_by(Beat) %>% 
  summarize(Mean_Response = mean(minute(response_time), na.rm = TRUE),
            count = n()) %>% 
  arrange(desc(Mean_Response))
...
opd_data_grouping %>% 
  mutate(response_time_minutes = hour(response_time)*60 + minute(response_time) + 1) -> opd_data_grouping


opd_data_grouping %>% 
  group_by(Beat, Grouping) %>% 
  summarize(Mean_Response = mean(dminutes(response_time), na.rm = TRUE),
            count = n()) %>% 
  arrange(desc(Mean_Response))  
...
shape1.sf %>% 
  left_join(mean_response_beat, c("cp_beat" = "Beat"), na_matches = "never") %>% 
...
shape_response.sf %>% 
  mutate(Mean_Response_Minutes = hour(Mean_Response)*60 + minute(Mean_Response)) -> shape_response.sf

#okay there is an issue where the codes for a couple of things arent mapping, but we can figure this out!

shape_response.sf %>% 
  ggplot(aes(fill = desc(Mean_Response_Minutes))) + 
  geom_sf() + 
  coord_sf() +
  theme_map()  
...
lmout <- lm_robust(response_time_minutes ~ Beat + factor(Priority), data = opd_data_grouping)
summary(lmout)

lmout2 <- lm_robust(response_time_minutes ~ Beat + Grouping + factor(Priority), data = opd_data_grouping)

lmout3 <- lm_robust(response_time_minutes ~ Beat + Grouping, data = opd_data_grouping)
summary(lmout3)
...
opd_data_grouping %>% 
  group_by(Beat) %>% 
  mutate(Mean_Response = mean(response_time_minutes, na.rm = TRUE)) %>% 
  ggplot() +
  geom_point(aes(x = Beat, y = Mean_Response)) +
