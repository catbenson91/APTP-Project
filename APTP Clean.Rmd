...
opd_data_new %>% 
   mutate(response_time_hms = hms::as.hms(response_time)) -> opd_data_new
...
fig2 <- plot_ly(pie_chart_perc_violent_crime, labels = ~`Violent Crime`, values = ~percent, type = 'pie')
fig2 <- fig %>% layout(title = 'Police Activity Related to Violent or Serious Crime',
...
opd_data_new %>% 
  ...
  theme_ipsum() +
    ...
    labs(title = "Response Time by Beat", subtitle = 'Higher Numbered Beat Run Farther East', caption = '2019 OPD Call Data') 
...
fig %>% 
  scale_colour_brewer(palette = "Reds") -> fig
